...
fn sharp_sgb() -> Matcher<SgbRom> {
    Matcher::new(r#"^(SYS-SGB-NT|SYS-SGB-2)\ ©\ 1994\ Nintendo\ (LH[[:alnum:]]{4})[[:alnum:]]{2}\ ([0-9]{2})([0-9]{2})\ [A-Z]$"#,
    move |c| {
        Ok(SgbRom {
            rom_code: c[1].to_owned(),
...
fn sharp_sgb2() -> Matcher<SgbRom> {
    Matcher::new(r#"^©\ 1998\ Nintendo\ (SYS-SGB2-10)\ (LH[[:alnum:]]{4})[[:alnum:]]{2}\ ([0-9]{2})([0-9]{2})\ [A-Z]$"#,
    move |c| {
        Ok(SgbRom {
            rom_code: c[1].to_owned(),
            manufacturer: Some(Manufacturer::Sharp),
...
fn oki() -> Matcher<SgbRom> {
    Matcher::new(r#"^(SYS-SGB-NT|SYS-SGB-2|SYS-SGB2-10)\ ©\ 1998\ Nintendo\ (M534011E)-[[:alnum:]]{2}\ ([0-9])([0-9]{2})[0-9]{3}[[:alnum:]]$"#,
    move |c| {
        Ok(SgbRom {
            rom_code: c[1].to_owned(),
