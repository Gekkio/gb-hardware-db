...
use super::{week2, year2_u16, Manufacturer, Matcher};
...
/// ```
fn sharp() -> Matcher<SgbRom> {
    Matcher::new(r#"^(SYS-SGB-(NT|2))\ ©\ 1994\ Nintendo\ (LH[[:alnum:]]{4})[[:alnum:]]{2}\ ([0-9]{2})([0-9]{2})\ [A-Z]$"#,
    move |c| {
        Ok(SgbRom {
            rom_code: c[1].to_owned(),
...
pub fn parse_sgb_rom(text: &str) -> Result<SgbRom, ()> {
    lazy_static! {
        static ref MATCHERS: [Matcher<SgbRom>; 5] =
            [toshiba(), sharp(), unknown(), unknown2(), unknown3(),];
    }
